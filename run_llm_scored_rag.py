...
def node_summary_targeted(n, keywords_or_slices):
    """
    Extract targeted sections from a node text based on keyword positions or slices.
    keywords_or_slices: list of (keyword, max_chars) tuples OR a single int for first N chars.
    Returns title + extracted text.
    """
    title = n.get("title", "")
    text = n.get("text", "")
    nid = n.get("node_id", "")
    ...
    parts = []
    first_part = text[:4000]  # always include start
    parts.append(first_part)

    for kw, limit in keywords_or_slices:
        idx = text.find(kw)
        if idx != -1 and idx > 4000:  # only if not already covered
            parts.append(f"\n[{kw.upper()}]\n" + text[idx:idx+limit])

    return f"[{nid}] {title}\n" + "\n".join(parts)


def build_context_targeted(nodes):
    """Build context with targeted extraction for each node."""
    parts = []
    for n in nodes:
        nid = n.get("node_id", "")
        if nid == "0252":
            parts.append(node_summary_targeted(n, [("outside electives", 3000)]))
        elif nid == "0649":
            parts.append(node_summary_targeted(n, [("Minor in Computer Science", 4000)]))
        else:
            parts.append(node_summary(n))  # use full summary for non-key nodes
    return "\n\n".join(parts)


def build_context_brief(nodes):
    """Build context using brief summaries for large nodes."""
    return "\n\n".join(node_summary_brief(n) for n in nodes)
...
def _audit_single(profile, category):
    ...
    equiv = load_equiv_table()

    # Build equivalency context for the prompt
    equiv_lines = []
    if equiv:
        equiv_lines.append("COURSE EQUIVALENCY TABLE:")
        for dept, courses in equiv.items():
            if dept.startswith("_"):
                continue
            for old_code, info in courses.items():
                official = info.get("official")
                status = info.get("equiv_status", "")
                note = info.get("note", "")
                if status == "not_equivalent":
                    equiv_lines.append(
                        f"  {old_code} is NOT equivalent to {official or '(none)'}: {note}"
                    )
                elif official:
                    equiv_lines.append(
                        f"  {old_code} = {official} ({info.get('title','')}) {note}"
                    )
        equiv_ctx = "\n".join(equiv_lines)
    else:
        equiv_ctx = "(no equivalency table loaded)"
    ...
    if category == "minor":
        ...
        major_nodes = [id_to_node[a] for a in ["0252", "0101", "0189"] if a in id_to_node]
        ctx = build_context_targeted(major_nodes)
        user = (
            "STUDENT PROFILE:\n" + student_ctx + "\n\n"
            "EQUIVALENCY TABLE:\n" + equiv_ctx + "\n\n"
            "BULLETIN SECTIONS:\n" + ctx + "\n\n"
            ...
            '"status": "COMPLETE|IN_PROGRESS|MISSING_REQUIREMENTS", '
            '"requirements": [{"course": "CSE 1301", "taken": true, "grade": "A", "equivalent": "CSE-E81 131"}], '
    ...
    print(f"  [{category}] calling minimax... ({len(user)} char prompt)")
    response = minimax(messages, max_tokens=4000)
    print(f"  [{category}] response: {len(response)} chars")
