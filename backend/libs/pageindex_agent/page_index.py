"""
Forked page_index.py from VectifyAI/PageIndex (MIT License).

Original: https://github.com/VectifyAI/PageIndex/blob/main/pageindex/page_index.py
Modification: Uses forked utils.py that routes LLM calls through Vertex AI Gemini.

This module generates hierarchical tree indexes from PDF documents.
"""
...
from pageindex_agent.utils import (
    ChatGPT_API,
    ChatGPT_API_async,
...
def extract_toc_content(content, model=None):
    ...
    new_response, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt2, chat_history=chat_history)
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        ...
        prompt3 = "please continue the generation of table of contents, directly output the remaining part of the structure"
        new_response, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt3, chat_history=chat_history)
        response = response + new_response
        if_complete = check_if_toc_transformation_is_complete(content, response, model)
...
def toc_transformer(toc_content, model=None):
    logger = logging.getLogger("pageindex_agent")
    logger.info("start toc_transformer")
    init_prompt = """
    ...
    logger.info("Sending initial toc_transformer request to LLM...")
    last_complete, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt)
    logger.info(f"toc_transformer initial response received. Finish reason: {finish_reason}")
    if_complete = check_if_toc_transformation_is_complete(toc_content, last_complete, model)
    logger.info(f"toc_transformer check_complete: {if_complete}")
    if if_complete == "yes" and finish_reason == "finished":
    ...
    while not (if_complete == "yes" and finish_reason == "finished"):
        retry_count += 1
        if retry_count > 10:
            print("Too many retries in toc_transformer, breaking loop.")
            break
        ...
        new_complete, finish_reason = ChatGPT_API_with_finish_reason(model=model, prompt=prompt2)
...
# =========================================================================
# TOC index extraction & page offset
# =========================================================================
...
# =========================================================================
# TOC generation from text (no existing TOC)
# =========================================================================
...
def validate_and_truncate_physical_indices(toc_with_page_number, page_list_length, start_index=1, logger=None):
    """Truncate physical_index values that exceed document length."""
...
def process_toc_with_page_numbers(toc_content, toc_page_list, page_list, toc_check_page_num=None, model=None, logger=None):
    ...
    if offset is None:
        # Can't calculate page offset — return empty so meta_processor falls back
        return []
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    _log.info(f"meta_processor: mode={mode}, start_index={start_index}, pages={len(page_list)}")

    if mode == "process_toc_with_page_numbers":
        _log.info("Calling process_toc_with_page_numbers...")
        ...
        _log.info("process_toc_with_page_numbers completed.")
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        ...
        if mode == "process_toc_with_page_numbers":
            ...
            # Final fallback exhausted — return best-effort result instead of crashing
            if toc_with_page_number:
                logger.info({
                    "message": "All processing modes exhausted; returning best-effort result",
                    "accuracy": accuracy,
                    "num_items": len(toc_with_page_number),
                })
                return toc_with_page_number
            raise Exception("Processing failed: all modes produced empty results")
...
async def process_large_node_recursively(node, page_list, opt=None, logger=None, _depth=0):
    ...
    if _depth >= MAX_RECURSION_DEPTH:
        logger.info(
            "Sub-node '%s' (pages %d-%d) hit max recursion depth %d — keeping as leaf" %
            (node.get("title", "?"), node["start_index"], node["end_index"], MAX_RECURSION_DEPTH)
        )
        return node
    ...
    if node["end_index"] - node["start_index"] > opt.max_page_num_each_node and token_num >= opt.max_token_num_each_node:
        ...
        try:
            ...
            logger.info(
                "Sub-node '%s' (pages %d-%d) failed recursive processing: %s — keeping as leaf" %
                (node.get("title", "?"), node["start_index"], node["end_index"], e)
            )
            return node
...
def page_index_main(doc, opt=None):
    """Generate a hierarchical tree index for a PDF document.

    Args:
        doc: Path to a PDF file or a BytesIO object.
        opt: Configuration namespace (from ConfigLoader.load()).

    Returns:
        dict with 'doc_name' and 'structure' (the tree).
    """
...
def page_index(
    ...
    """Convenience wrapper — builds config from kwargs and runs page_index_main."""
    user_opt = {
        arg: value
