...
# Hardcoded line numbers (1-indexed from earlier analysis):
# Line 162: def tree_retrieve
# Line 165: closing """
# Line 243: return answer, sources
# Line 284: # ── Routes

OLD_DEF = 161      # 0-indexed line 162
OLD_DOC_END = 164  # 0-indexed line 165
OLD_RETURN = 242   # 0-indexed line 243
OLD_ROUTES = 283   # 0-indexed line 284

# Verify
print(f"Line {OLD_DEF+1}: {repr(lines[OLD_DEF][:60])}")
print(f"Line {OLD_DOC_END+1}: {repr(lines[OLD_DOC_END][:60])}")
print(f"Line {OLD_RETURN+1}: {repr(lines[OLD_RETURN][:60])}")
print(f"Line {OLD_ROUTES+1}: {repr(lines[OLD_ROUTES][:60])}")

body_start = OLD_DOC_END + 1  # line after docstring
body_end = OLD_RETURN + 1    # line with return (inclusive)
...
print(f"Replacing body lines {body_start+1}-{body_end+1}")
new_lines = lines[:body_start] + [new_body] + lines[OLD_ROUTES:]
